...
class SentimentAnalyzer:
    ...
    def _format_news_for_analysis(self, news: List[Dict]) -> str:
        ...
        for i, article in enumerate(
            news[:15], 1
        ):  # Limit to 15 articles to avoid token limits
        ...
        try:
            ...
            if start_idx != -1 and end_idx > start_idx:
                ...
                return {
                    ...
                    "bullish_percent": data.get("sentiment_distribution", {}).get(
                        "bullish", 0.0
                    ),
                    "bearish_percent": data.get("sentiment_distribution", {}).get(
                        "bearish", 0.0
                    ),
                    "neutral_percent": data.get("sentiment_distribution", {}).get(
                        "neutral", 0.0
                    ),
    ...
    def news(self, days: int = 7, use_cache: bool = True) -> Dict:
        ...
        prompt = SENTIMENT_ANALYSIS_PROMPT.format(
            ticker=self.ticker, news_data=news_text
        )
        ...
        try:
            ...
            response_dict = ai_provider.analyze(
                data="", prompt=prompt, system_prompt=None  # No additional data needed
            )
