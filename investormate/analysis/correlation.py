...
class Correlation:
    ...
    def _fetch_price_data(self) -> pd.DataFrame:
        ...
        if failed_tickers:
            logger.warning("Failed to fetch data for: %s", ", ".join(failed_tickers))
    ...
    def find_diversification_candidates(
        ...
        if missing_portfolio:
            raise ValueError(
                f"Portfolio tickers not in correlation data: {missing_portfolio}"
            )
        ...
        if missing_universe:
            raise ValueError(
                f"Universe tickers not in correlation data: {missing_universe}"
            )
        ...
        for candidate in universe:
            ...
            correlations = [
                corr_matrix.loc[candidate, portfolio_stock]
