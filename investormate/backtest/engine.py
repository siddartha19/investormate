...
class BacktestEngine:
    ...
    def equity(self) -> float:
        ...
        position_value = (
            self.position_size * self.current_price if self.current_price else 0
        )
        ...
        try:
            ...
            if df.empty:
                raise DataFetchError(
                    f"No data found for {self.ticker} between {self.start_date} and {self.end_date}"
                )
        ...
        if shares is not None:
            ...
            if total_cost > self.cash:
                ...
                affordable_shares = int(
                    self.cash / (self.current_price * (1 + self.commission))
                )
    ...
    def run(self):
        ...
        for date in self.dates:
            ...
            try:
                ...
                logger.warning("Strategy error on %s: %s", date, e)
