...
def format_number(
    value: Any, decimals: int = 2, prefix: str = "", suffix: str = ""
) -> str:
    """
...
def print_ratios_table(ratios: Dict[str, Any], title: str = "Financial Ratios") -> None:
    ...
    for category, ratio_keys in categories.items():
        category_ratios = {
            k: v for k, v in ratios.items() if k in ratio_keys and v is not None
        }
        ...
        if category_ratios:
            ...
            for ratio_name, ratio_value in category_ratios.items():
                ...
                if any(
                    x in ratio_name
                    for x in ["margin", "yield", "growth", "roe", "roa", "roic", "wacc"]
                ):
