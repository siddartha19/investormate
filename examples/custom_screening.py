...
def example_function_based():
    ...
    def value_filter(stock):
        ...
        try:
            return (
                10 < stock.ratios.pe < 25  # Reasonable P/E
                and stock.ratios.roe > 0.15  # Strong ROE (15%+)
                and stock.price > 10  # Minimum price
            )
        except:
            return False
    ...
    strategy = CustomStrategy(
        filter_func=value_filter, rank_func=value_rank, universe=universe
    )
    ...
    for i, result in enumerate(results, 1):
        print(
            f"{i:<6} {result['ticker']:<10} {result['name'][:28]:<30} "
...
def example_builder_pattern():
    ...
    if results:
        ...
        for i, result in enumerate(results, 1):
            print(
                f"{i:<6} {result['ticker']:<10} ${result['price']:>9.2f} {result['rank']:>12.4f}"
            )
    else:
...
def example_growth_screening():
    ...
    def growth_filter(stock):
        ...
        try:
            return (
                stock.ratios.revenue_growth > 0.15  # 15%+ revenue growth
                and stock.ratios.roe > 0.20  # 20%+ ROE
                and stock.market_cap > 10_000_000_000  # $10B+ market cap
            )
        except:
            return False
    ...
    strategy = CustomStrategy(
        filter_func=growth_filter, rank_func=growth_rank, universe=universe
    )
    ...
    if results:
        ...
        for i, result in enumerate(results, 1):
            print(
                f"{i:<6} {result['ticker']:<10} {result['name'][:23]:<25} {result['rank']:>15.4f}"
            )
    else:
...
def example_dividend_screening():
    ...
    def dividend_filter(stock):
        ...
        try:
            return (
                stock.ratios.dividend_yield > 0.03  # 3%+ dividend yield
                and stock.ratios.payout_ratio < 0.70  # < 70% payout ratio (sustainable)
                and stock.ratios.pe < 25  # Reasonable valuation
            )
        except:
            return False
    ...
    def dividend_rank(stock):
        ...
        try:
            ...
            return (
                stock.ratios.dividend_yield / stock.ratios.payout_ratio
                if stock.ratios.payout_ratio > 0
                else 0
            )
        except:
            return 0
    ...
    strategy = CustomStrategy(
        filter_func=dividend_filter, rank_func=dividend_rank, universe=universe
    )
    ...
    if results:
        ...
        for i, result in enumerate(results, 1):
            print(
                f"{i:<6} {result['ticker']:<10} {result['name'][:23]:<25} {result['rank']:>12.4f}"
            )
    else:
...
def example_multi_criteria():
    ...
    def quality_filter(stock):
        ...
        try:
            return (
                stock.ratios.roe > 0.15  # Strong ROE
                and stock.ratios.current_ratio > 1.5  # Good liquidity
                and stock.ratios.debt_to_equity < 0.5  # Low debt
            )
        except:
            return False
    ...
    strategy = CustomStrategy(
        filter_func=quality_filter, rank_func=quality_rank, universe=universe
    )
    ...
    if results:
        ...
        for i, result in enumerate(results, 1):
            print(
                f"{i:<6} {result['ticker']:<10} {result['name'][:28]:<30} {result['rank']:>15.4f}"
            )
    else:
