...
def example_compare_stocks():
    ...
    print("-" * 70)
    print(
        f"{'Ticker':<10} {'Score':>8} {'Label':<15} {'Bullish%':>10} {'Articles':>10}"
    )
    print("-" * 70)
    ...
    for ticker in tickers:
        try:
            ...
            print(
                f"{ticker:<10} {sentiment['score']:>8.2f} {label:<15} "
...
def example_trading_signal():
    ...
    print(
        f"Confidence: {sentiment['bullish_percent']:.1f}% bullish, "
    ...
    for ticker in universe:
        try:
            ...
            if sentiment["score"] > 0.3:  # Filter for positive sentiment
                ...
                print(
                    f"✓ {ticker}: {sentiment['score']:.2f} ({sentiment['bullish_percent']:.1f}% bullish)"
                )
            else:
