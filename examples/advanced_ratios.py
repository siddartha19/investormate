...
def main():
    ...
    print(
        f"{'ROIC (Return on Invested Capital)'.ljust(40)}: {format_percentage(stock.ratios.roic).rjust(15)}"
    )
    print(
        f"{'ROE (Return on Equity)'.ljust(40)}: {format_percentage(stock.ratios.roe).rjust(15)}"
    )
    print(
        f"{'ROA (Return on Assets)'.ljust(40)}: {format_percentage(stock.ratios.roa).rjust(15)}"
    )
    ...
    print(
        f"{'WACC (Weighted Avg Cost of Capital)'.ljust(40)}: {format_percentage(stock.ratios.wacc).rjust(15)}"
    )
    print(
        f"{'Equity Multiplier (Financial Leverage)'.ljust(40)}: {format_number(stock.ratios.equity_multiplier, decimals=2).rjust(15)}"
    )
    print(
        f"{'Debt to Equity'.ljust(40)}: {format_number(stock.ratios.debt_to_equity, decimals=2).rjust(15)}"
    )
    print(
        f"{'Debt to Assets'.ljust(40)}: {format_percentage(stock.ratios.debt_to_assets).rjust(15)}"
    )
    ...
    if dupont:
        ...
        print(
            f"{'Reported ROE'.ljust(30)}: {format_percentage(dupont.get('roe')).rjust(15)}"
        )
        print(
            f"{'Profit Margin'.ljust(30)}: {format_percentage(dupont.get('profit_margin')).rjust(15)}"
        )
        print(
            f"{'Asset Turnover'.ljust(30)}: {format_number(dupont.get('asset_turnover'), decimals=3).rjust(15)}"
        )
        print(
            f"{'Equity Multiplier'.ljust(30)}: {format_number(dupont.get('equity_multiplier'), decimals=2).rjust(15)}"
        )
        print(
            f"{'Calculated ROE'.ljust(30)}: {format_percentage(dupont.get('calculated_roe')).rjust(15)}"
        )
